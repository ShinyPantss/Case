  ...
  return (
    <SafeAreaView className="flex-1 bg-transparent px-5 pt-[32px]  rounded-t-[16px] overflow-hidden">
      ...
      <View className="w-full flex-1 justify-end pb-6">
        <TouchableOpacity
          className={`mt-6 border border-[#d8dbe2]  rounded-full items-center p-5 flex-row justify-center gap-2 ${isLoading ? "opacity-50" : ""}`}
          onPress={handleWithdrawal}
        >
          <Text className="text-black text-center font-inter text-[18px] font-medium leading-[24px] tracking-[-0.27px]">
            {isLoading ? "Withdrawing..." : "Withdrawal"}
          </Text>
          <Image
            source={icons.minus}
            style={{ width: 20, height: 20, tintColor: "#020617" }}
          />
        </TouchableOpacity>
      </View>
    </SafeAreaView>
